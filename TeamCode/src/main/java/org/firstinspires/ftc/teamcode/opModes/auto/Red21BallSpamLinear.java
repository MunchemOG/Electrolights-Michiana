...
public class Red21BallSpamLinear extends NextFTCOpMode {
    ...
    private SequentialGroup shoot = new SequentialGroup(
            openTransfer, new Delay(0.05), transferOn, new Delay(0.3),
            new LambdaCommand().setStart(() -> Transfer.INSTANCE.setMotorPower(0)), closeTransfer
    );

    /** Aims at goal, updates flywheel + hood, and turns robot to face it. */
    private Command aimAtGoal = new LambdaCommand()
            .setStart(() -> {
    ...
    public class Paths {
        ...
        public Paths(Follower follower) {
            ...
            moverBacker = follower.pathBuilder()
                    .addPath(new BezierLine(new Pose(128, 64), new Pose(130, 59.75)))
            ...
            launchSet1 = follower.pathBuilder()
                    .addPath(new BezierLine(new Pose(126.665, 86.898), new Pose(92.000, 94.000)))
            ...
            launchSet3 = follower.pathBuilder()
                    .addPath(new BezierLine(new Pose(129.151, 36.938), new Pose(84, 103)))
                    .setLinearHeadingInterpolation(Math.toRadians(1), Math.toRadians(27))
                    .setVelocityConstraint(0.3).setTValueConstraint(0.8)
                    .addPoseCallback(new Pose(107, 69), reverseIntakeForMe, 0.7)
                    .addPoseCallback(new Pose(94, 88), shoot, 0.8).build();

            teleOpPark = follower.pathBuilder()
                    .addPath(new BezierLine(new Pose(84, 103), new Pose(91, 115)))
                    .setLinearHeadingInterpolation(Math.toRadians(30), Math.toRadians(90)).build();
        }
    }
}
